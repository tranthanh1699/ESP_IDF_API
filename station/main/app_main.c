...
void switch_data_callback(uint8_t * data, uint16_t len)
{
    if(*data == '1')
    {
        esp32_io_set_output(GPIO_NUM_2, 1);
    }
    else if(*data == '0')
    {
        esp32_io_set_output(GPIO_NUM_2, 0); 
    }
}
void slider_data_callback(uint8_t * data, uint16_t len)
{
    ...
    esp32_ledc_set_dutycycle(LEDC_CHANNEL_0, duty); 
}
...
void dht11_data_callback(void)
{
    ...
    char a = rand(); 
    char b = rand(); 
    sprintf(json_resp, "{\"temp\":\"%d\",\"humi\":\"%d\"}", a, b);
    dht11_response(json_resp, strlen(json_resp));
}
...
void app_main(void)
{
    ...
    ESP_ERROR_CHECK(ret);
    esp32_ledc_init(); 
    esp32_ledc_add_gpio(GPIO_NUM_2, LEDC_CHANNEL_0); 
